...
{
    [InitializeOnLoad]
    public class Zenith_ImportPanel : EditorWindow
    {
       private const string Url = "https://github.com/Zenith-Productions/Zenith-Unity-Kit/";
        ...
        public static void LoadJson()
        {
            ...
            dynamic configJson =
                JObject.Parse(File.ReadAllText(Zenith_Settings.projectConfigPath + Zenith_ImportManager.configName));
            ...
            foreach (var asset in assets)
            {
                ...
                else
                {
                    if (GUILayout.Button(
                        ...
                        " " + asset.Key))
                    {
                        Zenith_ImportManager.downloadAndImportAssetFromServer(asset.Value);
                    }

                    if (GUILayout.Button("Del", GUILayout.Width(40)))
                    {
                        Zenith_ImportManager.deleteAsset(asset.Value);
                    }
                }
                GUILayout.EndHorizontal();
            }
