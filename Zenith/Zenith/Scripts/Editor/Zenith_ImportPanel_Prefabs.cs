...
{
    [InitializeOnLoad]
    public class Zenith_ImportPanel_Prefabs : EditorWindow
    {
       private const string Url = "https://github.com/Zenith-Productions/ZPUK/";
        ...
        [MenuItem("Zenith/Import panel/Prefabs", false, 501)]
        public static void OpenImportPanel()
        {
            GetWindow<Zenith_ImportPanel_Prefabs>(true);
        }

        public void OnEnable()
        {
            titleContent = new GUIContent("Zenith Import panel prefabs");

            Zenith_ImportManager_Prefabs.checkForConfigUpdate();
            LoadJson();
            ...
            _chillHeader = new GUIStyle
            {
                normal =
                {
                    background = Resources.Load("ZenithHeader") as Texture2D,
        ...
        public static void LoadJson()
        {
            ...
            dynamic configJson =
                JObject.Parse(File.ReadAllText(Zenith_Settings.projectConfigPath + Zenith_ImportManager_Prefabs.configName));

            Debug.Log("Server Asset Url is: " + configJson["config"]["serverUrl"]);
            Zenith_ImportManager_Prefabs.serverUrl = configJson["config"]["serverUrl"].ToString();
            ...
            foreach (var asset in assets)
            {
                ...
                else
                {
                    if (GUILayout.Button(
                        ...
                        " " + asset.Key))
                    {
                        Zenith_ImportManager_Prefabs.downloadAndImportAssetFromServer(asset.Value);
                    }

                    if (GUILayout.Button("Del", GUILayout.Width(40)))
                    {
                        Zenith_ImportManager_Prefabs.deleteAsset(asset.Value);
                    }
                }
                GUILayout.EndHorizontal();
            }
