...
{
    [InitializeOnLoad]
    public class Zenith_ImportPanel_Avatars : EditorWindow
    {
       private const string Url = "https://github.com/Zenith-Productions/ZPUK/";
        ...
        [MenuItem("Zenith/Import panel/Avatars", false, 501)]
        public static void OpenImportPanel()
        {
            GetWindow<Zenith_ImportPanel_Avatars>(true);
        }

        public void OnEnable()
        {
            titleContent = new GUIContent("Zenith Import panel avatars");

            Zenith_ImportManager_Avatars.checkForConfigUpdate();
            LoadJson();
        ...
        public static void LoadJson()
        {
            ...
            dynamic configJson =
                JObject.Parse(File.ReadAllText(Zenith_Settings.projectConfigPath + Zenith_ImportManager_Avatars.configName));

            Debug.Log("Server Asset Url is: " + configJson["config"]["serverUrl"]);
            Zenith_ImportManager.serverUrl = configJson["config"]["serverUrl"].ToString();
            ...
            foreach (var asset in assets)
            {
                ...
                else
                {
                    if (GUILayout.Button(
                        ...
                        " " + asset.Key))
                    {
                        Zenith_ImportManager_Avatars.downloadAndImportAssetFromServer(asset.Value);
                    }

                    if (GUILayout.Button("Del", GUILayout.Width(40)))
                    {
                        Zenith_ImportManager_Avatars.deleteAsset(asset.Value);
                    }
                }
                GUILayout.EndHorizontal();
            }
