...
{
    [InitializeOnLoad]
    public class Zenith_ImportPanel_Shaders : EditorWindow
    {
       private const string Url = "https://github.com/Zenith-Productions/Zenith-Unity-Kit/";
        ...
        [MenuItem("Zenith/Import panel/Shaders", false, 501)]
        public static void OpenImportPanel()
        {
            GetWindow<Zenith_ImportPanel_Shaders>(true);
        }

        public void OnEnable()
        {
            titleContent = new GUIContent("Zenith Import panel shaders");

            Zenith_ImportManager_Shaders.checkForConfigUpdate();
            LoadJson();
        ...
        public static void LoadJson()
        {
            ...
            dynamic configJson =
                JObject.Parse(File.ReadAllText(Zenith_Settings.projectConfigPath + Zenith_ImportManager_Shaders.configName));

            Debug.Log("Server Asset Url is: " + configJson["config"]["serverUrl"]);
            Zenith_ImportManager_Shaders.serverUrl = configJson["config"]["serverUrl"].ToString();
            ...
            foreach (var asset in assets)
            {
                ...
                else
                {
                    if (GUILayout.Button(
                        ...
                        " " + asset.Key))
                    {
                        Zenith_ImportManager_Shaders.downloadAndImportAssetFromServer(asset.Value);
                    }

                    if (GUILayout.Button("Del", GUILayout.Width(40)))
                    {
                        Zenith_ImportManager_Shaders.deleteAsset(asset.Value);
                    }
                }
                GUILayout.EndHorizontal();
            }
